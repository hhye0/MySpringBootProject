...
//final 인 변수를 초기화하는 생성자를 자동으로 생성해주는 역할을 하는 롬북 어노테이션
...
//    }
    ...
    //목록조회 GET
    @GetMapping
    // 관리자 권한이 있는 사용자만 목록을 조회할 수 있음
    @PreAuthorize("hasAuthority('ROLE_ADMIN')")
    public List<User> getUsers() {
        return userRepository.findAll();
    }

    // 일반 사용자 권한이 있는 사용자만 목록을 조회할 수 있음
    ...
    public ResponseEntity<User> getUserById(@PathVariable Long id) {
        ...
        ResponseEntity<User> responseEntity = optionalUser.map(user -> ResponseEntity.ok(user)) //User 가 있는 경우 200 status code
...
//                .orElse(ResponseEntity.notFound().build());
    ...
    public User getUserByEmail(@PathVariable String email){
        ...
        User existUser = optionalUser.orElseThrow(() -> new BusinessException("User NotFound", HttpStatus.NOT_FOUND));
        return existUser;
    }
